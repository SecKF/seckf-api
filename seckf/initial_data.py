...
from seckf.database.checklists_results import ChecklistResult
from seckf.database.groups import Group
from seckf.database.lab_items import LabItem
from seckf.database.lab_items_code import LabItemCode
from seckf.database.lab_items_code_options import LabItemCodeOptions
...
from seckf.database.questions import Question
...
def load_initial_data(db):
    try:
        ...
        checklist = ChecklistType(name='Architecture, Design and Threat Modeling Requirements',
                                  description='Security architecture has almost become a lost art in many organizations. The days of the enterprise architect have passed in the age of DevSecOps. The application security field must catch up and adopt agile security principles while re-introducing leading security architecture principles to software practitioners. Architecture is not an implementation, but a way of thinking about a problem that has potentially many different answers, and no one single "correct" answer. All too often, security is seen as inflexible and demanding that developers fix code in a particular way, when the developers may know a much better way to solve the problem. There is no single, simple solution for architecture, and to pretend otherwise is a disservice to the software engineering field.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Authentication Verification Requirements',
                                  description='Authentication is the act of establishing, or confirming, someone (or something) as authentic and that claims made by a person or about a device are correct, resistant to impersonation, and prevent recovery or interception of passwords.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Session Management Verification Requirements',
                                  description='One of the core components of any web-based application or stateful API is the mechanism by which it controls and maintains the state for a user or device interacting with it. Session management changes a stateless protocol to stateful, which is critical for differentiating different users or devices.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Access Control Verification Requirements',
        ...
        checklist = ChecklistType(name='Validation, Sanitization and Encoding Verification Requirements',
                                  description='The most common web application security weakness is the failure to properly validate input coming from the client or the environment before directly using it without any output encoding. This weakness leads to almost all of the significant vulnerabilities in web applications, such as Cross-Site Scripting (XSS), SQL injection, interpreter injection, locale/Unicode attacks, file system attacks, and buffer overflows.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Stored Cryptography Verification Requirements',
                                  description='Ensure that a verified application satisfies the following high level requirements: • All cryptographic modules fail in a secure manner and that errors are handled correctly. • A suitable random number generator is used. • Access to keys is securely managed. V6.1 Data Classification',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Error Handling and Logging Verification Requirements',
                                  description='The primary objective of error handling and logging is to provide useful information for the user, administrators, and incident response teams. The objective is not to create massive amounts of logs, but high quality logs, with more signal than discarded noise.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Data Protection Verification Requirements',
                                  description='There are three key elements to sound data protection: Confidentiality, Integrity and Availability (CIA). This standard assumes that data protection is enforced on a trusted system, such as a server, which has been hardened and has sufficient protections.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Communications Verification Requirements',
                                  description='Leading industry advice on secure TLS configuration changes frequently, often due to catastrophic breaks in existing algorithms and ciphers. Always use the most recent versions of TLS configuration review tools (such as SSLyze or other TLS scanners) to configure the preferred order and algorithm selection. Configuration should be periodically checked to ensure that secure communications configuration is always present and effective.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Malicious Code Verification Requirements',
                                  description='Finding malicious code is proof of the negative, which is impossible to completely validate. Best efforts should be undertaken to ensure that the code has no inherent malicious code or unwanted functionality.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Business Logic Verification Requirements',
                                  description='Ensure that a verified application satisfies the following high level requirements: • The business logic flow is sequential, processed in order, and cannot be bypassed. • Business logic includes limits to detect and prevent automated attacks, such as continuous small funds transfers, or adding a million friends one at a time, and so on. • High value business logic flows have considered abuse cases and malicious actors, and have protections against spoofing, tampering, repudiation, information disclosure, and elevation of privilege attacks.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='File and Resources Verification Requirements',
                                  description='Ensure that a verified application satisfies the following high level requirements: • Untrusted file data should be handled accordingly and in a secure manner. • Untrusted file data obtained from untrusted sources are stored outside the web root and with limited permissions.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='API and Web Service Verification Requirements',
                                  description='Ensure that a verified application that uses trusted service layer APIs (commonly using JSON or XML or GraphQL) has: • Adequate authentication, session management and authorization of all web services. • Input validation of all parameters that transit from a lower to higher trust level. • Effective security controls for all API types, including cloud and Serverless API Please read this chapter in combination with all other chapters at this same level; we no longer duplicate authentication or API session management concerns.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()

        checklist = ChecklistType(name='Configuration Verification Requirements',
                                  description='Ensure that a verified application has: • A secure, repeatable, automatable build environment. • Hardened third party library, dependency and configuration management such that out of date or insecure components are not included by the application. • A secure-by-default configuration, such that administrators and users have to weaken the default security posture. Configuration of the application out of the box should be safe to be on the Internet, which means a safe out of the box configuration.',
                                  visibility=1)
        checklist.checklist_category_id = 1
        db.session.add(checklist)
        db.session.commit()
        ...
        # 29 -- 5.1
        q = Question(
            'Input Validation Requirements (parameter binding, url redirects, structure checking credit cards, phone numbers, etc, http request methods)',
            5)
        db.session.add(q)
        db.session.commit()
        ...
        # 30 -- 5.2
        q = Question(
            'Sanitization and Sandboxing Requirements (type/length checking, SSRF, wysiwyg/markdown/bbcode/etc)', 5)
        db.session.add(q)
        db.session.commit()
        ...
        # 39 -- 6.2
        q = Question(
            'Does your application need cryptgrapic functions (they need to be on par with the latest known to be secure standards)',
            6)
        db.session.add(q)
        db.session.commit()
        ...
        # 40 -- 6.3
        q = Question(
            'Does your application need secure random values or UUIDs (are they tested for sufficient entropy?)', 6)
        db.session.add(q)
        db.session.commit()
        ...
        # 43 -- 7.2
        q = Question(
            'Does the application needs to actively do logging (Do logs exist for authentication/ access control decisions?)',
            7)
        db.session.add(q)
        db.session.commit()
        ...
        # 44 -- 7.3
        q = Question(
            'Do you need to implement log protection (Are logs protected for un-authorized access, injection flaws(also in viewing software), and are timestamps synced to match respective timezones)',
            7)
        db.session.add(q)
        db.session.commit()
        ...
        # 45 -- 7.4
        q = Question(
            'Do you need centrilized error handling (Are errors on the application gracefully handled and show non generic error messages?)',
            7)
        db.session.add(q)
        db.session.commit()
        ...
        c = ChecklistKB('1.1.1',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Attack_Surface_Analysis_Cheat_Sheet.html',
                        2)
        c.question_id = 1
        c.kb_id = 272
        db.session.add(c)
        ...
        c = ChecklistKB('1.1.2',
                        ...
                        1, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Attack_Surface_Analysis_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1053',
                        2)
        c.question_id = 1
        c.kb_id = 1053
        db.session.add(c)
        ...
        c = ChecklistKB('1.1.3',
                        ...
                        1, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Attack_Surface_Analysis_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1110',
                        2)
        c.question_id = 1
        c.kb_id = 1110
        db.session.add(c)
        ...
        c = ChecklistKB('1.1.4',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Attack_Surface_Analysis_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1059',
                        2)
        c.question_id = 1
        c.kb_id = 1059
        db.session.add(c)
        ...
        c = ChecklistKB('1.1.6',
                        ...
                        1, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Attack_Surface_Analysis_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/637',
                        2)
        c.question_id = 1
        c.kb_id = 184
        db.session.add(c)
        ...
        c = ChecklistKB('1.2.1',
                        ...
                        1, False, 'https://cwe.mitre.org/data/definitions/250', 2)
        c.question_id = 2
        c.kb_id = 250
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('1.2.2',
                        ...
                        1, False, 'https://cwe.mitre.org/data/definitions/306', 2)
        c.question_id = 2
        c.kb_id = 306
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('1.4.1',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/602',
                        2)
        c.question_id = 3
        c.kb_id = 602
        db.session.add(c)
        ...
        c = ChecklistKB('1.4.2',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/284',
                        2)
        c.question_id = 3
        c.kb_id = 284
        db.session.add(c)
        ...
        c = ChecklistKB('1.4.3',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/272',
                        2)
        c.question_id = 3
        c.kb_id = 272
        db.session.add(c)
        ...
        c = ChecklistKB('1.4.5',
                        'Verify that attribute or feature-based access control is used whereby the code checks the user\s authorization for a feature/data item rather than just their role.',
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/275',
                        2)
        c.question_id = 3
        c.kb_id = 274
        db.session.add(c)
        ...
        c = ChecklistKB('1.5.1',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1029',
                        2)
        c.question_id = 4
        c.kb_id = 20
        db.session.add(c)
        ...
        c = ChecklistKB('1.5.2',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/502',
                        2)
        c.question_id = 4
        c.kb_id = 502
        db.session.add(c)
        ...
        c = ChecklistKB('1.5.3', 'Verify that input validation is enforced on a trusted service layer.', 1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/602',
                        2)
        c.question_id = 4
        c.kb_id = 602
        db.session.add(c)
        ...
        c = ChecklistKB('1.5.4',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/116',
                        2)
        c.question_id = 4
        c.kb_id = 116
        db.session.add(c)
        ...
        c = ChecklistKB('1.6.1',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/320',
                        2)
        c.question_id = 5
        c.kb_id = 320
        db.session.add(c)
        ...
        c = ChecklistKB('1.7.1', 'Verify that a common logging format and approach is used across the system.', 1,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1009',
                        2)
        c.question_id = 6
        c.kb_id = 1009
        db.session.add(c)
        ...
        c = ChecklistKB('1.7.2',
                        ...
                        1, False, 'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html', 2)
        c.question_id = 6
        c.kb_id = 2
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.8.1', 'Verify that all sensitive data is identified and classified into protection levels.',
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/User_Privacy_Protection_Cheat_Sheet.html',
                        2)
        c.question_id = 7
        c.kb_id = 278
        db.session.add(c)
        ...
        c = ChecklistKB('1.9.1',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/319',
                        2)
        c.question_id = 8
        c.kb_id = 319
        db.session.add(c)
        ...
        c = ChecklistKB('1.9.2',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/295',
                        2)
        c.question_id = 8
        c.kb_id = 295
        db.session.add(c)
        ...
        c = ChecklistKB('1.10.1',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Third_Party_Javascript_Management_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Virtual_Patching_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/284',
                        2)
        c.question_id = 9
        c.kb_id = 284
        db.session.add(c)
        ...
        c = ChecklistKB('1.11.1',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1059',
                        2)
        c.question_id = 70
        c.kb_id = 1059
        db.session.add(c)
        ...
        c = ChecklistKB('1.11.2',
                        ...
                        1, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/362',
                        2)
        c.question_id = 70
        c.kb_id = 362
        db.session.add(c)
        ...
        c = ChecklistKB('1.12.1', 'Verify that user-uploaded files are stored outside of the web root.', 1, False,
                        'https://cwe.mitre.org/data/definitions/552', 2)
        c.question_id = 71
        c.kb_id = 552
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.12.2',
                        ...
                        1, False, 'https://cwe.mitre.org/data/definitions/646', 2)
        c.question_id = 71
        c.kb_id = 646
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.14.1',
                        ...
                        1, False, 'https://cwe.mitre.org/data/definitions/923', 2)
        c.question_id = 72
        c.kb_id = 923
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.14.2',
                        ...
                        1, False, 'https://cwe.mitre.org/data/definitions/494', 2)
        c.question_id = 72
        c.kb_id = 494
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.14.3',
                        'Verify that the build pipeline warns of out-of-date or insecure components and takes appropriate actions.',
                        1, False, 'https://cwe.mitre.org/data/definitions/1104', 2)
        c.question_id = 72
        c.kb_id = 1104
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.14.4',
                        ...
                        1, False, 'https://cwe.mitre.org/', 2)
        c.question_id = 72
        c.kb_id = 281
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.14.5',
                        ...
                        1, False, 'https://cwe.mitre.org/data/definitions/265', 2)
        c.question_id = 72
        c.kb_id = 265
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.14.6',
                        ...
                        1, False, 'https://cwe.mitre.org/data/definitions/477', 2)
        c.question_id = 72
        c.kb_id = 477
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.1.1', 'Verify that user set passwords are at least 12 characters in length. (C6)', 2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Credential_Stuffing_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/521',
                        1)
        c.question_id = 10
        c.kb_id = 521
        db.session.add(c)
        ...
        c = ChecklistKB('2.1.2', 'Verify that passwords 64 characters or longer are permitted.', 2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Credential_Stuffing_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/521',
                        1)
        c.question_id = 10
        c.kb_id = 521
        ...
        c = ChecklistKB('2.1.5', 'Verify users can change their password.', 2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Credential_Stuffing_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/620',
                        1)
        c.question_id = 10
        c.kb_id = 620
        db.session.add(c)
        ...
        c = ChecklistKB('2.1.6',
                        'Verify that password change functionality requires the user\s current and new password.', 2,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Credential_Stuffing_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/620',
                        1)
        c.question_id = 10
        ...
        c = ChecklistKB('2.1.8',
                        'Verify that a password strength meter is provided to help users set a stronger password.', 2,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Credential_Stuffing_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/521',
                        1)
        c.question_id = 10
        ...
        c = ChecklistKB('2.1.10',
                        'Verify that there are no periodic credential rotation or password history requirements.', 2,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Credential_Stuffing_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/263',
                        1)
        c.question_id = 10
        c.kb_id = 263
        db.session.add(c)
        ...
        c = ChecklistKB('2.2.1',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/307',
                        1)
        c.question_id = 11
        c.kb_id = 307
        db.session.add(c)
        ...
        c = ChecklistKB('2.2.2',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/304',
                        1)
        c.question_id = 11
        c.kb_id = 304
        db.session.add(c)
        ...
        c = ChecklistKB('2.2.3',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/620',
                        1)
        c.question_id = 11
        c.kb_id = 620
        db.session.add(c)
        ...
        c = ChecklistKB('2.2.4',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/308',
                        3)
        c.question_id = 11
        c.kb_id = 308
        db.session.add(c)
        ...
        c = ChecklistKB('2.2.5',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/319',
                        3)
        c.question_id = 11
        c.kb_id = 319
        db.session.add(c)
        ...
        c = ChecklistKB('2.3.1',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/330', 1)
        c.question_id = 12
        c.kb_id = 330
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.3.2',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/308', 2)
        c.question_id = 12
        c.kb_id = 308
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.3.3',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/287', 2)
        c.question_id = 12
        c.kb_id = 287
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.4.1',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/916',
                        2)
        c.question_id = 13
        c.kb_id = 916
        db.session.add(c)
        ...
        c = ChecklistKB('2.5.1',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/640',
                        1)
        c.question_id = 14
        c.kb_id = 640
        db.session.add(c)
        ...
        c = ChecklistKB('2.5.3', 'Verify password credential recovery does not reveal the current password in any way.',
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/640',
                        1)
        c.question_id = 14
        ...
        c = ChecklistKB('2.5.4', 'Verify shared or default accounts are not present (e.g. "root", "admin", or "sa").',
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/16',
                        1)
        c.question_id = 14
        c.kb_id = 276
        db.session.add(c)
        ...
        c = ChecklistKB('2.5.5',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/304',
                        1)
        c.question_id = 14
        c.kb_id = 304
        db.session.add(c)
        ...
        c = ChecklistKB('2.5.7',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/308',
                        2)
        c.question_id = 14
        c.kb_id = 308
        db.session.add(c)
        ...
        c = ChecklistKB('2.6.1', 'Verify that lookup secrets can be used only once.', 2, False,
        ...
        c = ChecklistKB('2.6.2',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/330', 2)
        c.question_id = 15
        c.kb_id = 330
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.6.3',
                        'Verify that lookup secrets are resistant to offline attacks, such as predictable values.', 2,
                        False, 'https://cwe.mitre.org/data/definitions/310', 2)
        c.question_id = 15
        c.kb_id = 310
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.7.1',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/287',
                        1)
        c.question_id = 16
        c.kb_id = 287
        db.session.add(c)
        ...
        c = ChecklistKB('2.7.4',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/523',
                        1)
        c.question_id = 16
        c.kb_id = 523
        db.session.add(c)
        ...
        c = ChecklistKB('2.7.5',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/256',
                        2)
        c.question_id = 16
        c.kb_id = 256
        db.session.add(c)
        ...
        c = ChecklistKB('2.7.6',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/310',
                        2)
        c.question_id = 16
        c.kb_id = 310
        db.session.add(c)
        ...
        c = ChecklistKB('2.8.1', 'Verify that time-based OTPs have a defined lifetime before expiring.', 2, False,
                        'https://cwe.mitre.org/data/definitions/613', 1)
        c.question_id = 17
        c.kb_id = 613
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.8.2',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/320', 2)
        c.question_id = 17
        c.kb_id = 320
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.8.3',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/326', 2)
        c.question_id = 17
        c.kb_id = 326
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('2.8.4', 'Verify that time-based OTP can be used only once within the validity period.', 2,
                        False, 'https://cwe.mitre.org/data/definitions/287', 2)
        c.question_id = 17
        c.kb_id = 287
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.8.6',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/613', 2)
        c.question_id = 17
        c.kb_id = 613
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.8.7',
                        'Verify that biometric authenticators are limited to use only as 308 5.2.3 secondary factors in conjunction with either something you have and something you know.',
                        2, False, 'https://cwe.mitre.org/data/definitions/308', 3)
        c.question_id = 17
        c.kb_id = 308
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('2.9.2',
                        ...
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/330',
                        2)
        c.question_id = 18
        c.kb_id = 330
        db.session.add(c)
        ...
        c = ChecklistKB('2.9.3',
                        'Verify that approved cryptographic algorithms are used in the generation, seeding, and verification.',
                        2, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/327',
                        2)
        c.question_id = 18
        c.kb_id = 327
        db.session.add(c)
        ...
        c = ChecklistKB('2.10.1',
                        'Verify that integration secrets do not rely on unchanging passwords, such as API keys or shared privileged accounts.',
                        2, False,
                        "https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/287",
                        2)
        c.question_id = 19
        c.kb_id = 287
        db.session.add(c)
        ...
        c = ChecklistKB('2.10.2', 'Verify that if passwords are required, the credentials are not a default account', 2,
                        False, 'https://cwe.mitre.org/data/definitions/255', 2)
        c.question_id = 19
        c.kb_id = 287
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('2.10.3',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/522', 2)
        c.question_id = 19
        c.kb_id = 522
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('2.10.4',
                        ...
                        2, False, 'https://cwe.mitre.org/data/definitions/798', 2)
        c.question_id = 19
        c.kb_id = 798
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('3.1.1',
                        'Verify the application never reveals session tokens in URL parameters or error messages.', 3,
                        False, 'https://cwe.mitre.org/data/definitions/598', 1)
        c.question_id = 20
        c.kb_id = 598
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('3.2.1', 'Verify the application generates a new session token on user authentication.', 3,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/384',
                        1)
        c.question_id = 21
        c.kb_id = 384
        db.session.add(c)
        ...
        c = ChecklistKB('3.2.2', 'Verify that session tokens possess at least 64 bits of entropy.', 3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/311',
                        1)
        c.question_id = 21
        c.kb_id = 311
        db.session.add(c)
        ...
        c = ChecklistKB('3.2.3',
                        ...
                        3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/539',
                        1)
        c.question_id = 21
        c.kb_id = 539
        db.session.add(c)
        ...
        c = ChecklistKB('3.2.4', 'Verify that session token are generated using approved cryptographic algorithms', 3,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/331',
                        2)
        c.question_id = 21
        c.kb_id = 331
        db.session.add(c)
        ...
        c = ChecklistKB('3.3.1',
                        ...
                        3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/613',
                        1)
        c.question_id = 22
        c.kb_id = 613
        db.session.add(c)
        ...
        c = ChecklistKB('3.4.1', 'Verify that cookie-based session tokens have the \Secure\ attribute set.', 3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/614',
                        1)
        c.question_id = 23
        c.kb_id = 614
        db.session.add(c)
        ...
        c = ChecklistKB('3.4.2', 'Verify that cookie-based session tokens have the \HttpOnly\ attribute set.', 3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1004',
                        1)
        c.question_id = 23
        c.kb_id = 1004
        db.session.add(c)
        ...
        c = ChecklistKB('3.4.3',
                        ...
                        3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/16',
                        1)
        c.question_id = 23
        c.kb_id = 2
        db.session.add(c)
        ...
        c = ChecklistKB('3.5.1',
                        ...
                        3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/290',
                        2)
        c.question_id = 24
        c.kb_id = 290
        db.session.add(c)
        ...
        c = ChecklistKB('3.5.2',
                        ...
                        3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/798',
                        2)
        c.question_id = 24
        c.kb_id = 798
        db.session.add(c)
        ...
        c = ChecklistKB('3.5.3',
                        ...
                        3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/345',
                        2)
        c.question_id = 24
        c.kb_id = 345
        db.session.add(c)
        ...
        c = ChecklistKB('3.7.1',
                        ...
                        3, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transaction_Authorization_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/778',
                        1)
        c.question_id = 25
        c.kb_id = 778
        db.session.add(c)
        ...
        c = ChecklistKB('4.1.1',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Access_Control_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Testing_Automation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/602',
                        1)
        c.question_id = 26
        c.kb_id = 602
        db.session.add(c)
        ...
        c = ChecklistKB('4.1.2',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Access_Control_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Testing_Automation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/639',
                        1)
        c.question_id = 26
        c.kb_id = 639
        db.session.add(c)
        ...
        c = ChecklistKB('4.1.3',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Access_Control_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Testing_Automation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/285',
                        1)
        c.question_id = 26
        c.kb_id = 285
        db.session.add(c)
        ...
        c = ChecklistKB('4.1.4',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Access_Control_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Testing_Automation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/276',
                        1)
        c.question_id = 26
        c.kb_id = 276
        db.session.add(c)
        ...
        c = ChecklistKB('4.1.5', 'Verify that access controls fail securely including when an exception occurs.', 4,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Access_Control_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Testing_Automation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/285',
                        1)
        c.question_id = 26
        ...
        c = ChecklistKB('4.2.1',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Insecure_Direct_Object_Reference_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Testing_Automation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/639',
                        1)
        c.question_id = 27
        c.kb_id = 639
        db.session.add(c)
        ...
        c = ChecklistKB('4.2.2',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Insecure_Direct_Object_Reference_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Testing_Automation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/352',
                        1)
        c.question_id = 27
        c.kb_id = 352
        db.session.add(c)
        ...
        c = ChecklistKB('4.3.1',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/419',
                        1)
        c.question_id = 28
        c.kb_id = 419
        db.session.add(c)
        ...
        c = ChecklistKB('4.3.2',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/548',
                        1)
        c.question_id = 28
        c.kb_id = 548
        db.session.add(c)
        ...
        c = ChecklistKB('4.3.3',
                        ...
                        4, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/732',
                        1)
        c.question_id = 28
        c.kb_id = 732
        db.session.add(c)
        ...
        c = ChecklistKB('5.1.1',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Mass_Assignment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/235',
                        1)
        c.question_id = 29
        c.kb_id = 235
        db.session.add(c)
        ...
        c = ChecklistKB('5.1.2',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Mass_Assignment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/915',
                        1)
        c.question_id = 29
        c.kb_id = 915
        db.session.add(c)
        ...
        c = ChecklistKB('5.1.3',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Mass_Assignment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/20',
                        1)
        c.question_id = 29
        c.kb_id = 20
        db.session.add(c)
        ...
        c = ChecklistKB('5.1.5',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Mass_Assignment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/601',
                        1)
        c.question_id = 29
        c.kb_id = 601
        db.session.add(c)
        ...
        c = ChecklistKB('5.2.1',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/116',
                        1)
        c.question_id = 30
        c.kb_id = 116
        db.session.add(c)
        ...
        c = ChecklistKB('5.2.2',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/138',
                        1)
        c.question_id = 30
        c.kb_id = 138
        db.session.add(c)
        ...
        c = ChecklistKB('5.2.3',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/147',
                        1)
        c.question_id = 30
        c.kb_id = 147
        db.session.add(c)
        ...
        c = ChecklistKB('5.2.4',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/95',
                        1)
        c.question_id = 30
        c.kb_id = 95
        db.session.add(c)
        ...
        c = ChecklistKB('5.2.5',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/94',
                        1)
        c.question_id = 30
        c.kb_id = 94
        db.session.add(c)
        ...
        c = ChecklistKB('5.2.6',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/918',
                        1)
        c.question_id = 30
        c.kb_id = 918
        db.session.add(c)
        ...
        c = ChecklistKB('5.2.7',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/159',
                        1)
        c.question_id = 30
        c.kb_id = 159
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.1',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/116',
                        1)
        c.question_id = 31
        c.kb_id = 116
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.2',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/176',
                        1)
        c.question_id = 31
        c.kb_id = 176
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.3',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/79',
                        1)
        c.question_id = 31
        c.kb_id = 79
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.4',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/89',
                        1)
        c.question_id = 33
        c.kb_id = 89
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.6',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/830',
                        1)
        c.question_id = 31
        c.kb_id = 830
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.7',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/943',
                        1)
        c.question_id = 34
        c.kb_id = 943
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.8',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/78',
                        1)
        c.question_id = 35
        c.kb_id = 78
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.9',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/829',
                        1)
        c.question_id = 32
        c.kb_id = 829
        db.session.add(c)
        ...
        c = ChecklistKB('5.3.10',
                        'Verify that the application protects against XPath injection or XML injection attacks.', 5,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/HTML5_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Injection_Prevention_in_Java_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/LDAP_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Query_Parameterization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Bean_Validation_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/643',
                        1)
        c.question_id = 36
        c.kb_id = 643
        db.session.add(c)
        ...
        c = ChecklistKB('5.5.1',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/502',
                        1)
        c.question_id = 37
        c.kb_id = 502
        db.session.add(c)
        ...
        c = ChecklistKB('5.5.2',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/611',
                        1)
        c.question_id = 37
        c.kb_id = 611
        db.session.add(c)
        ...
        c = ChecklistKB('5.5.4',
                        ...
                        5, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/95',
                        1)
        c.question_id = 37
        c.kb_id = 95
        db.session.add(c)
        ...
        c = ChecklistKB('6.1.1',
                        'Verify that regulated private data is stored encrypted while at rest, such as personally identifiable information (PII), sensitive personal information, or data assessed likely to be subject to EUs GDPR.',
                        6, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/User_Privacy_Protection_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/311',
                        2)
        c.question_id = 38
        c.kb_id = 311
        db.session.add(c)
        ...
        c = ChecklistKB('6.2.1',
                        ...
                        6, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/310',
                        1)
        c.question_id = 39
        c.kb_id = 310
        db.session.add(c)
        ...
        c = ChecklistKB('6.2.3',
                        ...
                        6, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/326',
                        2)
        c.question_id = 39
        c.kb_id = 326
        db.session.add(c)
        ...
        c = ChecklistKB('6.3.1',
                        'Verify that all random numbers, random file names, random GUIDs, and random strings are generated using the cryptographic modules approved cryptographically secure random number generator when these random values are intended to be not guessable by an attacker.',
                        6, False, 'https://cwe.mitre.org/data/definitions/338', 2)
        c.question_id = 40
        c.kb_id = 338
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('6.3.3',
                        ...
                        6, False, 'https://cwe.mitre.org/data/definitions/338', 3)
        c.question_id = 40
        c.kb_id = 338
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('6.4.1',
                        ...
                        6, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/798',
                        2)
        c.question_id = 41
        c.kb_id = 798
        db.session.add(c)
        ...
        c = ChecklistKB('6.4.2',
                        ...
                        6, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/320',
                        2)
        c.question_id = 41
        c.kb_id = 320
        db.session.add(c)
        ...
        c = ChecklistKB('7.1.1',
                        ...
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/532',
                        1)
        c.question_id = 42
        c.kb_id = 532
        db.session.add(c)
        ...
        c = ChecklistKB('7.1.3',
                        ...
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/778',
                        2)
        c.question_id = 42
        c.kb_id = 778
        db.session.add(c)
        ...
        c = ChecklistKB('7.2.2',
                        ...
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/285',
                        2)
        c.question_id = 43
        c.kb_id = 285
        db.session.add(c)
        ...
        c = ChecklistKB('7.3.1',
                        ...
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/117',
                        2)
        c.question_id = 44
        c.kb_id = 117
        db.session.add(c)
        ...
        c = ChecklistKB('7.3.2',
                        'Verify that all events are protected from injection when viewed in log viewing software', 7,
                        False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/117',
                        2)
        c.question_id = 44
        ...
        c = ChecklistKB('7.3.3', 'Verify that security logs are protected from unauthorized access and modification.',
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/200',
                        2)
        c.question_id = 44
        c.kb_id = 200
        db.session.add(c)
        ...
        c = ChecklistKB('7.3.4',
                        ...
                        7, False, 'https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html', 2)
        c.question_id = 44
        c.kb_id = 2
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('7.4.1',
                        ...
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Error_Handling_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/210',
                        1)
        c.question_id = 45
        c.kb_id = 210
        db.session.add(c)
        ...
        c = ChecklistKB('7.4.2',
                        ...
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Error_Handling_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/544',
                        2)
        c.question_id = 45
        c.kb_id = 544
        db.session.add(c)
        ...
        c = ChecklistKB('7.4.3',
                        ...
                        7, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Error_Handling_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/460',
                        2)
        c.question_id = 45
        c.kb_id = 460
        db.session.add(c)
        ...
        c = ChecklistKB('8.1.1',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/532', 2)
        c.question_id = 46
        c.kb_id = 532
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.1.2',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/524', 2)
        c.question_id = 19
        c.kb_id = 524
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.1.3',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/233', 2)
        c.question_id = 46
        c.kb_id = 233
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.1.4',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/525', 2)
        c.question_id = 46
        c.kb_id = 525
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.1.5',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/770', 3)
        c.question_id = 46
        c.kb_id = 770
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.1.6',
                        'Verify that backups are stored securely to prevent data from being stolen or corrupted.', 8,
                        False, 'https://cwe.mitre.org/data/definitions/19', 3)
        c.question_id = 46
        c.kb_id = 300
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.2.1',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/525', 1)
        c.question_id = 25
        c.kb_id = 525
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.2.2',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/922', 1)
        c.question_id = 47
        c.kb_id = 922
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.3.1',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/319', 1)
        c.question_id = 48
        c.kb_id = 319
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('8.3.2', 'Verify that users have a method to remove or export their data on demand.', 8, False,
                        'https://cwe.mitre.org/data/definitions/212', 1)
        c.question_id = 48
        c.kb_id = 212
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.3.3',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/285', 1)
        c.question_id = 48
        c.kb_id = 285
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.3.4',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/200', 1)
        c.question_id = 48
        c.kb_id = 200
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.3.6',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/226', 2)
        c.question_id = 48
        c.kb_id = 226
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('8.3.8',
                        ...
                        8, False, 'https://cwe.mitre.org/data/definitions/285', 2)
        c.question_id = 48
        c.kb_id = 285
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('9.1.1',
                        ...
                        9, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/HTTP_Strict_Transport_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/319',
                        1)
        c.question_id = 49
        c.kb_id = 319
        db.session.add(c)
        ...
        c = ChecklistKB('9.1.2',
                        ...
                        9, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/HTTP_Strict_Transport_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Protection_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/TLS_Cipher_String_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/326',
                        1)
        c.question_id = 49
        c.kb_id = 326
        db.session.add(c)
        ...
        c = ChecklistKB('9.2.1',
                        ...
                        9, False, 'https://cwe.mitre.org/data/definitions/295', 2)
        c.question_id = 50
        c.kb_id = 295
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('9.2.2',
                        ...
                        9, False, 'https://cwe.mitre.org/data/definitions/319', 2)
        c.question_id = 50
        c.kb_id = 319
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('9.2.3',
                        ...
                        9, False, 'https://cwe.mitre.org/data/definitions/297', 2)
        c.question_id = 50
        c.kb_id = 297
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('9.2.4',
                        ...
                        9, False, 'https://cwe.mitre.org/data/definitions/299', 2)
        c.question_id = 50
        c.kb_id = 299
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('9.2.5', 'Verify that backend TLS connection failures are logged', 9, False,
                        'https://cwe.mitre.org/data/definitions/544', 3)
        c.question_id = 50
        c.kb_id = 544
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('10.1.1',
                        ...
                        10, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Third_Party_Javascript_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/749',
                        3)
        c.question_id = 51
        c.kb_id = 749
        db.session.add(c)
        ...
        c = ChecklistKB('10.2.1',
                        'Verify that the application source code and third party libraries do not contain unauthorized phone home or data collection capabilities. Where such functionality exists, obtain the users permission for it to operate before collecting any data.',
                        10, False, 'https://cwe.mitre.org/data/definitions/359', 2)
        c.question_id = 52
        c.kb_id = 359
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('10.2.2',
                        ...
                        10, False, 'https://cwe.mitre.org/data/definitions/272', 2)
        c.question_id = 52
        c.kb_id = 272
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('10.2.3',
                        ...
                        10, False, 'https://cwe.mitre.org/data/definitions/507', 3)
        c.question_id = 52
        c.kb_id = 507
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('10.2.4',
                        ...
                        10, False, 'https://cwe.mitre.org/data/definitions/511', 3)
        c.question_id = 52
        c.kb_id = 511
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('10.2.5',
        ...
        c = ChecklistKB('10.3.1',
                        ...
                        10, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/16',
                        1)
        c.question_id = 53
        c.kb_id = 2
        db.session.add(c)
        ...
        c = ChecklistKB('10.3.2',
                        ...
                        10, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/353',
                        1)
        c.question_id = 53
        c.kb_id = 353
        db.session.add(c)
        ...
        c = ChecklistKB('10.3.3',
                        ...
                        10, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/350',
                        1)
        c.question_id = 53
        c.kb_id = 350
        db.session.add(c)
        ...
        c = ChecklistKB('11.1.1',
                        ...
                        11, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/841',
                        1)
        c.question_id = 54
        c.kb_id = 841
        db.session.add(c)
        ...
        c = ChecklistKB('11.1.2',
                        ...
                        11, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/779',
                        1)
        c.question_id = 54
        c.kb_id = 779
        db.session.add(c)
        ...
        c = ChecklistKB('11.1.3',
                        ...
                        11, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/770',
                        1)
        c.question_id = 54
        c.kb_id = 770
        db.session.add(c)
        ...
        c = ChecklistKB('11.1.5',
                        ...
                        11, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/841',
                        1)
        c.question_id = 54
        c.kb_id = 841
        db.session.add(c)
        ...
        c = ChecklistKB('11.1.6',
                        ...
                        11, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/367',
                        2)
        c.question_id = 54
        c.kb_id = 367
        db.session.add(c)
        ...
        c = ChecklistKB('11.1.7',
                        ...
                        11, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/754',
                        2)
        c.question_id = 54
        c.kb_id = 754
        db.session.add(c)
        ...
        c = ChecklistKB('11.1.8',
                        'Verify the application has configurable alerting when automated attacks or unusual activity is detected.',
                        11, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/390',
                        2)
        c.question_id = 54
        c.kb_id = 390
        db.session.add(c)
        ...
        c = ChecklistKB('12.1.1',
                        'Verify that the application will not accept large files that could fill up storage or cause a denial of service attack.',
                        12, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/400',
                        1)
        c.question_id = 55
        c.kb_id = 400
        db.session.add(c)
        ...
        c = ChecklistKB('12.1.2',
                        ...
                        12, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/400',
                        2)
        c.question_id = 55
        c.kb_id = 400
        db.session.add(c)
        ...
        c = ChecklistKB('12.1.3',
                        ...
                        12, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/409',
                        2)
        c.question_id = 55
        c.kb_id = 409
        db.session.add(c)
        ...
        c = ChecklistKB('12.2.1',
                        'Verify that files obtained from untrusted sources are validated to be of expected type based on the files content.',
                        12, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Third_Party_Javascript_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/434',
                        2)
        c.question_id = 56
        c.kb_id = 434
        db.session.add(c)
        ...
        c = ChecklistKB('12.3.1',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/22', 1)
        c.question_id = 57
        c.kb_id = 22
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.3.2',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/73', 1)
        c.question_id = 57
        c.kb_id = 73
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.3.3',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/98', 1)
        c.question_id = 57
        c.kb_id = 98
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.3.4',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/641', 1)
        c.question_id = 57
        c.kb_id = 641
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.3.5',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/78', 1)
        c.question_id = 57
        c.kb_id = 78
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.3.6',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/829', 2)
        c.question_id = 57
        c.kb_id = 829
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.4.1',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/922', 1)
        c.question_id = 58
        c.kb_id = 922
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.4.2',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/509', 1)
        c.question_id = 58
        c.kb_id = 509
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.5.1',
                        ...
                        12, False, 'https://cwe.mitre.org/data/definitions/552', 1)
        c.question_id = 59
        c.kb_id = 552
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.5.2',
                        'Verify that direct requests to uploaded files will never be executed as HTML/JavaScript content.',
                        12, False, 'https://cwe.mitre.org/data/definitions/434', 1)
        c.question_id = 59
        c.kb_id = 434
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('12.6.1',
                        ...
                        12, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/918',
                        1)
        c.question_id = 60
        c.kb_id = 918
        db.session.add(c)
        ...
        c = ChecklistKB('13.1.1',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Web_Service_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/116',
                        1)
        c.question_id = 61
        c.kb_id = 116
        db.session.add(c)
        ...
        c = ChecklistKB('13.1.2',
                        'Verify that access to administration and management functions is limited to authorized administrators.',
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Web_Service_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/419',
                        1)
        c.question_id = 61
        c.kb_id = 419
        db.session.add(c)
        ...
        c = ChecklistKB('13.1.3',
                        'Verify API URLs do not expose sensitive information, such as the API key, session tokens etc.',
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Web_Service_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/598',
                        1)
        c.question_id = 61
        c.kb_id = 598
        db.session.add(c)
        ...
        c = ChecklistKB('13.1.4',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Web_Service_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/285',
                        2)
        c.question_id = 61
        c.kb_id = 285
        db.session.add(c)
        ...
        c = ChecklistKB('13.1.5',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Web_Service_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/434',
                        2)
        c.question_id = 61
        c.kb_id = 434
        db.session.add(c)
        ...
        c = ChecklistKB('13.2.1',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/650',
                        1)
        c.question_id = 62
        c.kb_id = 650
        db.session.add(c)
        ...
        c = ChecklistKB('13.2.2', 'Verify that JSON schema validation is in place and verified before accepting input.',
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/20',
                        1)
        c.question_id = 62
        c.kb_id = 20
        db.session.add(c)
        ...
        c = ChecklistKB('13.2.3',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/352',
                        1)
        c.question_id = 62
        c.kb_id = 352
        db.session.add(c)
        ...
        c = ChecklistKB('13.2.4',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/779',
                        2)
        c.question_id = 62
        c.kb_id = 779
        db.session.add(c)
        ...
        c = ChecklistKB('13.2.5',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/436',
                        2)
        c.question_id = 62
        c.kb_id = 436
        db.session.add(c)
        ...
        c = ChecklistKB('13.2.6',
                        'Verify that the message headers and payload are trustworthy and not modified in transit. Requiring strong encryption for transport (TLS only) may be sufficient in many cases as it provides both confidentiality and integrity protection. Per- message digital signatures can provide additional assurance on top of the transport protections for high-security applications but bring with them additional complexity and risks to weigh against the benefits',
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/REST_Assessment_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/598',
                        2)
        c.question_id = 62
        c.kb_id = 598
        db.session.add(c)
        ...
        c = ChecklistKB('13.3.1',
                        ...
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/20',
                        1)
        c.question_id = 63
        c.kb_id = 20
        db.session.add(c)
        ...
        c = ChecklistKB('13.3.2',
                        'Verify that the message payload is signed using WS-Security to ensure reliable transport between client and service.',
                        13, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/XML_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/345',
                        2)
        c.question_id = 63
        c.kb_id = 345
        db.session.add(c)
        ...
        c = ChecklistKB('13.4.1',
                        ...
                        13, False, 'https://cwe.mitre.org/data/definitions/770', 2)
        c.question_id = 64
        c.kb_id = 770
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('13.4.2',
                        ...
                        13, False, 'https://cwe.mitre.org/data/definitions/285', 2)
        c.question_id = 64
        c.kb_id = 285
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('14.1.2',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/120',
                        2)
        c.question_id = 65
        c.kb_id = 120
        db.session.add(c)
        ...
        c = ChecklistKB('14.1.5',
                        ...
                        14, False, 'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html', 3)
        c.question_id = 65
        c.kb_id = 2
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('14.2.1',
                        'Verify that all components are up to date, preferably using a dependency checker during build or compile time.',
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1104',
                        1)
        c.question_id = 65
        c.kb_id = 1104
        db.session.add(c)
        ...
        c = ChecklistKB('14.2.2',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/531',
                        1)
        c.question_id = 66
        c.kb_id = 531
        db.session.add(c)
        ...
        c = ChecklistKB('14.2.4',
                        'Verify that third party components come from pre-defined, trusted and continually maintained repositories',
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/828',
                        2)
        c.question_id = 66
        c.kb_id = 828
        db.session.add(c)
        ...
        c = ChecklistKB('14.2.5', 'Verify that an inventory catalog is maintained of all third party libraries in use.',
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html',
                        2)
        c.question_id = 66
        c.kb_id = 2
        db.session.add(c)
        ...
        c = ChecklistKB('14.2.6',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/265',
                        2)
        c.question_id = 66
        c.kb_id = 265
        db.session.add(c)
        ...
        c = ChecklistKB('14.3.1',
                        ...
                        14, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Error_Handling_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/209',
                        1)
        c.question_id = 67
        c.kb_id = 209
        db.session.add(c)
        ...
        c = ChecklistKB('14.3.2',
                        ...
                        14, True,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Error_Handling_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/497',
                        1)
        c.question_id = 67
        c.kb_id = 497
        db.session.add(c)
        ...
        c = ChecklistKB('14.3.3',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Error_Handling_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/200',
                        1)
        c.question_id = 67
        c.kb_id = 200
        db.session.add(c)
        ...
        c = ChecklistKB('14.4.1',
                        'Verify that every HTTP response contains a content type header specifying a safe character set (e.g., UTF-8, ISO 8859-1).',
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/173',
                        1)
        c.question_id = 68
        c.kb_id = 173
        db.session.add(c)
        ...
        c = ChecklistKB('14.4.2',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/116',
                        1)
        c.question_id = 68
        c.kb_id = 116
        db.session.add(c)
        ...
        c = ChecklistKB('14.4.3',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1021',
                        1)
        c.question_id = 68
        c.kb_id = 1021
        db.session.add(c)
        ...
        c = ChecklistKB('14.4.4', 'Verify that all responses contain X-Content-Type-Options: nosniff.', 14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/116',
                        1)
        c.question_id = 68
        c.kb_id = 116
        ...
        c = ChecklistKB('14.4.5',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/523',
                        1)
        c.question_id = 68
        c.kb_id = 523
        db.session.add(c)
        ...
        c = ChecklistKB('14.4.7',
                        ...
                        14, False,
                        'https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/346',
                        1)
        c.question_id = 68
        c.kb_id = 346
        db.session.add(c)
        ...
        c = ChecklistKB('14.5.1',
                        ...
                        14, False, 'https://cwe.mitre.org/data/definitions/749', 1)
        c.question_id = 69
        c.kb_id = 749
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('14.5.2',
                        ...
                        14, False, 'https://cwe.mitre.org/data/definitions/346', 1)
        c.question_id = 69
        c.kb_id = 346
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('14.5.3',
        ...
        c = ChecklistKB('14.5.4',
                        ...
                        14, False, 'https://cwe.mitre.org/data/definitions/306', 2)
        c.question_id = 69
        c.kb_id = 306
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('1.1', 'All app components are identified and known to be needed.', 15, True,
        ...
        c = ChecklistKB('1.4', 'Data considered sensitive in the context of the mobile app is clearly identified.', 15,
                        False, '', 1)
        c.kb_id = 2004
        c.question_id = 79
        db.session.add(c)
        ...
        c = ChecklistKB('1.6',
                        ...
                        15, True,
                        'https://owasp.org/www-community/Threat_Modeling,https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html',
                        2)
        c.kb_id = 2006
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('1.7', 'All security controls have a centralized implementation.', 15, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05h-Testing-Platform-Interaction.md#testing-for-insecure-configuration-of-instant-apps-mstg-arch-1-mstg-arch-7',
                        2)
        c.kb_id = 2007
        c.question_id = 73
        ...
        c = ChecklistKB('1.9', 'A mechanism for enforcing updates of the mobile app exists.', 15, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06h-Testing-Platform-Interaction.md#testing-enforced-updating-mstg-arch-9,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05h-Testing-Platform-Interaction.md#testing-enforced-updating-mstg-arch-9',
                        2)
        c.kb_id = 2009
        c.question_id = 78
        db.session.add(c)
        ...
        c = ChecklistKB('1.10', 'Security is addressed within all parts of the software development lifecycle.', 15,
                        False, '', 2)
        c.kb_id = 2010
        c.question_id = 80
        db.session.add(c)
        ...
        c = ChecklistKB('1.11', 'A responsible disclosure policy is in place and effectively applied.', 15, False, '',
                        2)
        c.kb_id = 2011
        c.question_id = 77
        db.session.add(c)
        ...
        c = ChecklistKB('2.3', 'No sensitive data is written to application logs.', 16, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05d-Testing-Data-Storage.md#testing-logs-for-sensitive-data-mstg-storage-3,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06d-Testing-Data-Storage.md#checking-logs-for-sensitive-data-mstg-storage-3',
                        1)
        c.kb_id = 2015
        c.question_id = 84
        db.session.add(c)
        ...
        c = ChecklistKB('2.5', 'The keyboard cache is disabled on text inputs that process sensitive data.', 16, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05d-Testing-Data-Storage.md#determining-whether-the-keyboard-cache-is-disabled-for-text-input-fields-mstg-storage-5,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06d-Testing-Data-Storage.md#finding-sensitive-data-in-the-keyboard-cache-mstg-storage-5',
                        1)
        c.kb_id = 2017
        c.question_id = 84
        db.session.add(c)
        ...
        c = ChecklistKB('2.6', 'No sensitive data is exposed via IPC mechanisms.', 16, False,
        ...
        c = ChecklistKB('2.7', 'No sensitive data, such as passwords or pins, is exposed through the user interface.',
                        16, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05d-Testing-Data-Storage.md#checking-for-sensitive-data-disclosure-through-the-user-interface-mstg-storage-7,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06d-Testing-Data-Storage.md#checking-for-sensitive-data-disclosed-through-the-user-interface-mstg-storage-7',
                        1)
        c.kb_id = 2024
        c.question_id = 85
        db.session.add(c)
        ...
        c = ChecklistKB('2.8', 'No sensitive data is included in backups generated by the mobile operating system.', 16,
                        False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05d-Testing-Data-Storage.md#testing-backups-for-sensitive-data-mstg-storage-8,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06d-Testing-Data-Storage.md#testing-backups-for-sensitive-data-mstg-storage-8',
                        2)
        c.kb_id = 2019
        c.question_id = 84
        db.session.add(c)
        ...
        c = ChecklistKB('2.9', 'The app removes sensitive data from views when moved to the background.', 16, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05d-Testing-Data-Storage.md#finding-sensitive-information-in-auto-generated-screenshots-mstg-storage-9,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06d-Testing-Data-Storage.md#testing-auto-generated-screenshots-for-sensitive-information-mstg-storage-9',
                        2)
        c.kb_id = 2020
        c.question_id = 85
        db.session.add(c)
        ...
        c = ChecklistKB('2.10',
                        ...
                        16, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05d-Testing-Data-Storage.md#checking-memory-for-sensitive-data-mstg-storage-10,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06d-Testing-Data-Storage.md#testing-memory-for-sensitive-data-mstg-storage-10',
                        2)
        c.kb_id = 2021
        c.question_id = 84
        db.session.add(c)
        ...
        c = ChecklistKB('2.12',
                        ...
                        16, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04i-Testing-user-interaction.md#testing-user-education-mstg-storage-12',
                        2)
        c.kb_id = 2023
        c.question_id = 86
        db.session.add(c)
        ...
        c = ChecklistKB('3.2', 'The app uses proven implementations of cryptographic primitives.', 17, False,
        ...
        c = ChecklistKB('3.5', 'The app doesn\'t re-use the same cryptographic key for multiple purposes.', 17, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06e-Testing-Cryptography.md#testing-key-management-mstg-crypto-1-and-mstg-crypto-5,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05e-Testing-Cryptography.md#testing-the-purposes-of-keys-mstg-crypto-5',
                        2)
        c.kb_id = 2029
        c.question_id = 88
        db.session.add(c)
        ...
        c = ChecklistKB('3.6', 'All random values are generated using a sufficiently secure random number generator.',
                        17, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06e-Testing-Cryptography.md#testing-random-number-generation-mstg-crypto-6,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05e-Testing-Cryptography.md#testing-random-number-generation-mstg-crypto-6',
                        2)
        c.kb_id = 2030
        c.question_id = 107
        db.session.add(c)
        ...
        c = ChecklistKB('4.2',
                        ...
                        18, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04e-Testing-Authentication-and-Session-Management.md#testing-stateful-session-management-mstg-auth-2',
                        1)
        c.kb_id = 2032
        c.question_id = 91
        db.session.add(c)
        ...
        c = ChecklistKB('4.4', 'The remote endpoint terminates the existing session when the user logs out.', 18, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04e-Testing-Authentication-and-Session-Management.md#testing-user-logout-mstg-auth-4',
                        1)
        c.kb_id = 2034
        c.question_id = 91
        db.session.add(c)
        ...
        c = ChecklistKB('4.5', 'A password policy exists and is enforced at the remote endpoint.', 18, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04e-Testing-Authentication-and-Session-Management.md#testing-best-practices-for-passwords-mstg-auth-5-and-mstg-auth-6',
                        1)
        c.kb_id = 2035
        c.question_id = 90
        db.session.add(c)
        ...
        c = ChecklistKB('4.7',
                        ...
                        18, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04e-Testing-Authentication-and-Session-Management.md#testing-session-timeout-mstg-auth-7',
                        1)
        c.kb_id = 2037
        c.question_id = 91
        db.session.add(c)
        ...
        c = ChecklistKB('4.8',
                        ...
                        18, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06f-Testing-Local-Authentication.md#testing-local-authentication-mstg-auth-8-and-mstg-storage-11,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05f-Testing-Local-Authentication.md#testing-biometric-authentication-mstg-auth-8',
                        2)
        c.kb_id = 2038
        c.question_id = 90
        db.session.add(c)
        ...
        c = ChecklistKB('4.10', 'Sensitive transactions require step-up authentication.', 18, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04e-Testing-Authentication-and-Session-Management.md#testing-two-factor-authentication-and-step-up-authentication-mstg-auth-9-and-mstg-auth-10',
                        2)
        c.kb_id = 2039
        c.question_id = 90
        ...
        c = ChecklistKB('4.11',
                        ...
                        18, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04e-Testing-Authentication-and-Session-Management.md#testing-login-activity-and-device-blocking-mstg-auth-11',
                        2)
        c.kb_id = 2040
        c.question_id = 89
        db.session.add(c)
        ...
        c = ChecklistKB('4.12', 'Authorization models should be defined and enforced at the remote endpoint.', 18,
                        False, '', 1)
        c.kb_id = 2079
        c.question_id = 90
        db.session.add(c)
        ...
        c = ChecklistKB('5.1',
                        ...
                        19, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06g-Testing-Network-Communication.md#app-transport-security-mstg-network-2,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04f-Testing-Network-Communication.md#verifying-data-encryption-on-the-network-mstg-network-1-and-mstg-network-2',
                        1)
        c.kb_id = 2042
        c.question_id = 92
        db.session.add(c)
        ...
        c = ChecklistKB('5.5',
                        ...
                        19, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x04f-Testing-Network-Communication.md#making-sure-that-critical-operations-use-secure-communication-channels-mstg-network-5',
                        2)
        c.kb_id = 2045
        c.question_id = 93
        db.session.add(c)
        ...
        c = ChecklistKB('5.6', 'The app only depends on up-to-date connectivity and security libraries.', 19, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05g-Testing-Network-Communication.md#testing-the-security-provider-mstg-network-6',
                        2)
        c.kb_id = 2091
        c.question_id = 94
        db.session.add(c)
        ...
        c = ChecklistKB('6.1', 'The app only requests the minimum set of permissions necessary.', 20, True,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06h-Testing-Platform-Interaction.md#testing-app-permissions-mstg-platform-1,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05h-Testing-Platform-Interaction.md#testing-app-permissions-mstg-platform-1',
                        1)
        c.kb_id = 2047
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('6.3',
                        ...
                        20, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06h-Testing-Platform-Interaction.md#testing-custom-url-schemes-mstg-platform-3,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05h-Testing-Platform-Interaction.md#testing-custom-url-schemes-mstg-platform-3',
                        1)
        c.kb_id = 2049
        c.question_id = 96
        db.session.add(c)
        ...
        c = ChecklistKB('6.5', 'JavaScript is disabled in WebViews unless explicitly required.', 20, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06h-Testing-Platform-Interaction.md#testing-ios-webviews-mstg-platform-5,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05h-Testing-Platform-Interaction.md#testing-javascript-execution-in-webviews-mstg-platform-5',
                        1)
        c.kb_id = 2051
        c.question_id = 104
        db.session.add(c)
        ...
        c = ChecklistKB('6.6',
                        ...
                        20, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06h-Testing-Platform-Interaction.md#testing-webview-protocol-handlers-mstg-platform-6,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05h-Testing-Platform-Interaction.md#testing-webview-protocol-handlers-mstg-platform-6',
                        1)
        c.kb_id = 2052
        c.question_id = 105
        db.session.add(c)
        ...
        c = ChecklistKB('6.8', 'Object deserialization, if any, is implemented using safe serialization APIs.', 20,
                        False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06h-Testing-Platform-Interaction.md#testing-object-persistence-mstg-platform-8,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05h-Testing-Platform-Interaction.md#testing-object-persistence-mstg-platform-8',
                        1)
        c.kb_id = 2054
        c.question_id = 106
        db.session.add(c)
        ...
        c = ChecklistKB('6.9', 'The app protects itself against screen overlay attacks. (Android only)', 20, False,
        ...
        c = ChecklistKB('7.3', 'Debugging symbols have been removed from native binaries.', 21, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05i-Testing-Code-Quality-and-Build-Settings.md#testing-for-debugging-symbols-mstg-code-3,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06i-Testing-Code-Quality-and-Build-Settings.md#finding-debugging-symbols-mstg-code-3',
                        1)
        c.kb_id = 2060
        c.question_id = 100
        db.session.add(c)
        ...
        c = ChecklistKB('7.5',
                        ...
                        21, True,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05i-Testing-Code-Quality-and-Build-Settings.md#checking-for-weaknesses-in-third-party-libraries-mstg-code-5,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06i-Testing-Code-Quality-and-Build-Settings.md#checking-for-weaknesses-in-third-party-libraries-mstg-code-5',
                        1)
        c.kb_id = 2062
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('7.6', 'The app catches and handles possible exceptions.', 21, False,
        ...
        c = ChecklistKB('7.7', 'Error handling logic in security controls denies access by default.', 21, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05i-Testing-Code-Quality-and-Build-Settings.md#testing-exception-handling-mstg-code-6-and-mstg-code-7,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06i-Testing-Code-Quality-and-Build-Settings.md#testing-exception-handling-mstg-code-6',
                        1)
        c.kb_id = 2063
        c.question_id = 99
        ...
        c = ChecklistKB('7.8', 'In unmanaged code, memory is allocated, freed and used securely.', 21, False,
        ...
        c = ChecklistKB('8.1',
                        ...
                        22, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06j-Testing-Resiliency-Against-Reverse-Engineering.md#jailbreak-detection-mstg-resilience-1,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05j-Testing-Resiliency-Against-Reverse-Engineering.md#testing-root-detection-mstg-resilience-1',
                        3)
        c.kb_id = 2067
        c.question_id = 102
        db.session.add(c)
        ...
        c = ChecklistKB('8.5', 'The app detects, and responds to, being run in an emulator.', 22, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05j-Testing-Resiliency-Against-Reverse-Engineering.md#testing-emulator-detection-mstg-resilience-5,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06j-Testing-Resiliency-Against-Reverse-Engineering.md#testing-emulator-detection-mstg-resilience-5',
                        3)
        c.kb_id = 2071
        c.question_id = 102
        db.session.add(c)
        ...
        c = ChecklistKB('8.6', 'The app detects, and responds to, tampering the code and data in its own memory space.',
                        22, False,
        ...
        c = ChecklistKB('8.9',
                        ...
                        22, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05j-Testing-Resiliency-Against-Reverse-Engineering.md#testing-obfuscation-mstg-resilience-9,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06j-Testing-Resiliency-Against-Reverse-Engineering.md#testing-obfuscation-mstg-resilience-9',
                        3)
        c.kb_id = 2075
        c.question_id = 101
        db.session.add(c)
        ...
        c = ChecklistKB('8.10',
                        ...
                        22, False,
                        'https://github.com/OWASP/owasp-mstg/blob/master/Document/0x05j-Testing-Resiliency-Against-Reverse-Engineering.md#testing-device-binding-mstg-resilience-10,https://github.com/OWASP/owasp-mstg/blob/master/Document/0x06j-Testing-Resiliency-Against-Reverse-Engineering.md#device-binding-mstg-resilience-10',
                        3)
        c.kb_id = 2076
        c.question_id = 101
        db.session.add(c)
        ...
        p = Project('Design Patterns ASVS LvL 1', '4.0', 'Application Security Verification Standard',
                    '2021-10-05 13:37')
        db.session.add(p)
        db.session.commit()
        ...
        p = ProjectSprint('Session Management Verification Requirements',
        ...
        p = ProjectSprint('Validation, Sanitization and Encoding Verification Requirements',
        ...
        p = ProjectSprint('Stored Cryptography Verification Requirements',
        ...
        p = ProjectSprint('Error Handling and Logging Verification Requirements',
        ...
        p = ProjectSprint('File and Resources Verification Requirements',
        ...
        p = ProjectSprint('API and Web Service Verification Requirements',
        ...
        p = Project('Design Patterns ASVS LvL 2', '4.0', 'Application Security Verification Standard',
                    '2021-10-05 13:37')
        db.session.add(p)
        db.session.commit()

        # Add sprints for the ASVS LvL 1 checklist category
        p = ProjectSprint('Architecture, Design and Threat Modeling Requirements',
        ...
        p = Project('Design Patterns ASVS LvL 3', '4.0', 'Application Security Verification Standard',
                    '2021-10-05 13:37')
        db.session.add(p)
        db.session.commit()
        ...
        lab_code_sol = LabItemCode(
            "<?php $query  = 'SELECT id, name FROM products ORDER BY name LIMIT 20 OFFSET $offset;'; ?>", 14,
            "hint: think hard", "php")
        db.session.add(lab_code_sol)
        db.session.commit()

        lab_code_sol = LabItemCode("System.Diagnostics.Process.Start('CMD.exe',strCommand);", 2, "hint: think hard",
                                   "asp")
        db.session.add(lab_code_sol)
        db.session.commit()
        ...
        db.session.add(
            LabItem('Path traversal (LFI)', 'https://owasp-skf.gitbook.io/asvs-write-ups/local-file-inclusion-1-lfi-1',
                    1, 'lfi', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Cross Site Scripting', 'https://owasp-skf.gitbook.io/asvs-write-ups/cross-site-scripting-xss', 1,
                    'xss', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Cross site scripting (attribute)',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/cross-site-scripting-attribute-xss-attribute',
                               1, 'xss-attribute', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Cross site scripting (href)',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/cross-site-scripting-href-xss-href', 1,
                               'xss-url', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('XSSI', 'https://owasp-skf.gitbook.io/asvs-write-ups/untrusted-sources-xssi', 2,
                               'untrusted-sources-js', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Cross site request forgery', 'https://owasp-skf.gitbook.io/asvs-write-ups/csrf', 3, 'csrf',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Cross site request forgery (same site)',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/csrf-samesite', 3, 'csrf-samesite',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Cross site request forgery weak', 'https://owasp-skf.gitbook.io/asvs-write-ups/csrf-weak', 2,
                    'csrf-weak', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('External entity attack', 'https://owasp-skf.gitbook.io/asvs-write-ups/xml-external-entity-xxe', 2,
                    'xxe', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Insecure file upload', 'https://owasp-skf.gitbook.io/asvs-write-ups/file-upload', 1, 'file-upload',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Clickjacking', 'https://owasp-skf.gitbook.io/asvs-write-ups/clickjacking', 1, 'clickjacking',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Rate-limiting', 'https://owasp-skf.gitbook.io/asvs-write-ups/ratelimiting-brute-force-login', 1,
                    'ratelimiting', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('HttpOnly (session hijacking)',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/httponly-session-hijacking-xss', 3,
                               'session-hijacking-xss', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('SQLI (union select)', 'https://owasp-skf.gitbook.io/asvs-write-ups/sqli-union', 2, 'sqli',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Open redirect', 'https://owasp-skf.gitbook.io/asvs-write-ups/url-redirection', 1,
                               'url-redirection', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Open redirect ( harder )', 'https://owasp-skf.gitbook.io/asvs-write-ups/url-redirection-harder', 2,
                    'url-redirection-harder', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Open redirect ( hard )', 'https://owasp-skf.gitbook.io/asvs-write-ups/url-redirection-harder-2', 3,
                    'url-redirection-harder2', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('CORS exploitation', 'https://owasp-skf.gitbook.io/asvs-write-ups/cors-exploitation', 3, 'cors',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Formulla injection', 'https://owasp-skf.gitbook.io/asvs-write-ups/formula-injection', 1,
                               'formula-injection', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Mass assingment attack', 'https://owasp-skf.gitbook.io/asvs-write-ups/parameter-binding', 1,
                    'parameter-binding', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('SQLI -like', 'https://owasp-skf.gitbook.io/asvs-write-ups/sqli-like', 2, 'sqli-like', 'SKF-labs',
                    None))
        db.session.commit()

        db.session.add(
            LabItem('SQLI-blind', 'https://owasp-skf.gitbook.io/asvs-write-ups/sqli-blind', 3, 'sqli-blind', 'SKF-labs',
                    None))
        db.session.commit()

        db.session.add(
            LabItem('Remote file inclusion', 'https://owasp-skf.gitbook.io/asvs-write-ups/remote-file-inclusion-rfi', 1,
                    'rfi', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Local file inclusion ( harder )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/local-file-inclusion-2-lfi-2', 1, 'lfi-2',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Local file inclusion ( hard )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/local-file-inclusion-3-lfi-3', 1, 'lfi-3',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Content security policiy',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/content-security-policy-csp', 1, 'csp',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Server side request forgery',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/server-side-request-forgery-ssrf', 3,
                               'ssrf', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Server side template injection',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/server-side-template-injection-ssti', 3,
                               'ssti', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Insecure direct object reference',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/insecure-direct-object-references-idor', 2,
                               'idor', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('JWT null', 'https://owasp-skf.gitbook.io/asvs-write-ups/jwt-null', 2, 'jwt-null', 'SKF-labs',
                    None))
        db.session.commit()

        db.session.add(
            LabItem('JWT weak secret', 'https://owasp-skf.gitbook.io/asvs-write-ups/jwt-secret', 2, 'jwt-secret',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Insecure deserialization (yaml)',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/deserialisation-yaml-des-yaml', 3,
                               'des-yaml', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Insecure deserialization pickle ( hard )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/deserialisation-pickle-des-pickle', 3,
                               'des-pickle', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Insecure deserialization pickle ( harder )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/deserialisation-pickle-2-des-pickle-2', 3,
                               'des-pickle-2', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Race condition', 'https://owasp-skf.gitbook.io/asvs-write-ups/race-condition', 3, 'racecondition',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Regex Ddos', 'https://owasp-skf.gitbook.io/asvs-write-ups/dos-regex', 1, 'dos-regex', 'SKF-labs',
                    None))
        db.session.commit()

        db.session.add(
            LabItem('Command injection', 'https://owasp-skf.gitbook.io/asvs-write-ups/command-injection-cmd', 1, 'cmd',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Command injection ( easy )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/command-injection-2-cmd-2', 1, 'cmd2',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Command injection ( harder )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/command-injection-3-cmd-3', 2, 'cmd3',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Command injection ( hard )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/command-injection-4-cmd-4', 3, 'cmd4',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Command injection ( blind )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/command-injection-blind-cmd-blind', 3,
                               'cmd-blind', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Information disclosure 1',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/information-leakeage-in-comments', 1,
                               'info-leakeage-comments', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Information disclosure 2',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/information-leakeage-in-metadata', 1,
                               'info-leakeage-metadata', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Authentication bypass ( easy )', 'https://owasp-skf.gitbook.io/asvs-write-ups/auth-bypass', 1,
                    'auth-bypass-simple', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Authentication bypass', 'https://owasp-skf.gitbook.io/asvs-write-ups/auth-bypass-1', 1,
                               'auth-bypass', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Authentication bypass ( harder )', 'https://owasp-skf.gitbook.io/asvs-write-ups/auth-bypass-2', 2,
                    'auth-bypass-2', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Authentication bypass ( hard )', 'https://owasp-skf.gitbook.io/asvs-write-ups/auth-bypass-3', 3,
                    'auth-bypass-3', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Right to left override attack',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/right-to-left-override-rtlo', 1, 'rtlo',
                               'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Session puzzeling', 'https://owasp-skf.gitbook.io/asvs-write-ups/kbid-250-session-puzzling', 3,
                    'sessionpuzzle', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Graphql DOS', 'https://owasp-skf.gitbook.io/asvs-write-ups/graphql-dos', 3,
                               'graphql-dos-resource-exhaustion', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('GraphQL IDOR', 'https://owasp-skf.gitbook.io/asvs-write-ups/graphql-idor', 3, 'graphql-idor',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('GraphQL Injections', 'https://owasp-skf.gitbook.io/asvs-write-ups/graphql-injections', 3,
                    'graphql-injections', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('GraphQL Introspection', 'https://owasp-skf.gitbook.io/asvs-write-ups/graphql-introspection', 3,
                    'graphql-info-introspection', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('GraphQL Mutations', 'https://owasp-skf.gitbook.io/asvs-write-ups/graphql-mutations', 3,
                               'graphql-mutation', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Client side template injection',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/client-side-template-injection-csti', 2,
                               'csti', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Prototype pollution', 'https://owasp-skf.gitbook.io/asvs-write-ups/prototype-pollution', 3,
                    'prototype', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('CSS Injection', 'https://owasp-skf.gitbook.io/asvs-write-ups/css-injection-cssi', 2, 'cssi',
                    'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Client side restriction bypass',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/client-side-restriction-bypass', 1,
                               'client-side-restriction-bypass', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Client side restriction bypass ( harder )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/client-side-restriction-bypass-harder/kbid-xxx-client-side-restriction-bypass-2',
                               2, 'client-side-restriction-bypass-2', 'SKF-labs', None))
        db.session.commit()

        db.session.add(
            LabItem('Credentials guessing ( easy )', 'https://owasp-skf.gitbook.io/asvs-write-ups/credentials-guessing',
                    2, 'credentials-guessing-1', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Credentials guessing ( harder )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/credentials-guessing-2', 2,
                               'credentials-guessing-2', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('Credentials guessing ( hard )',
                               'https://owasp-skf.gitbook.io/asvs-write-ups/credentials-guessing-3', 2,
                               'credentials-guessing-3', 'SKF-labs', None))
        db.session.commit()

        db.session.add(LabItem('API-only XSS',
                               'https://pwning.owasp-juice.shop/part2/xss.html#perform-a-persisted-xss-attack-without-using-the-frontend-application-at-all',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Access Log',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#gain-access-to-any-access-log-file-of-the-server',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Admin Registration',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#register-as-a-user-with-administrator-privileges',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Admin Section',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#access-the-administration-section-of-the-store',
                               2, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Arbitrary File Write',
                               'https://pwning.owasp-juice.shop/part2/vulnerable-components.html#overwrite-the-legal-information-file',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Bjoerns Favorite Pet',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#reset-the-password-of-bjoerns-owasp-account-via-the-forgot-password-mechanism',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Blockchain Hype',
                               'https://pwning.owasp-juice.shop/part2/security-through-obscurity.html#learn-about-the-token-sale-before-its-official-announcement',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Blocked RCE DoS',
                               'https://pwning.owasp-juice.shop/part2/insecure-deserialization.html#perform-a-remote-code-execution-that-would-keep-a-less-hardened-application-busy-forever',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('CAPTCHA Bypass',
                               'https://pwning.owasp-juice.shop/part2/broken-anti-automation.html#submit-10-or-more-customer-feedbacks-within-10-seconds',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Change Benders Password',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#change-benders-password-into-slurmcl4ssic-without-using-sql-injection-or-forgot-password',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Christmas Special',
                               'https://pwning.owasp-juice.shop/part2/injection.html#order-the-christmas-special-offer-of-2014',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('CSP Bypass',
                               'https://pwning.owasp-juice.shop/part2/xss.html#bypass-the-content-security-policy-and-perform-an-xss-attack-on-a-legacy-page',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Client-side XSS Protection',
                               'https://pwning.owasp-juice.shop/part2/xss.html#perform-a-persisted-xss-attack-bypassing-a-client-side-security-mechanism',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Confidential Document',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#access-a-confidential-document',
                               1, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('DOM XSS', 'https://pwning.owasp-juice.shop/part2/xss.html#perform-a-dom-xss-attack', 1,
                               'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Database Schema',
                               'https://pwning.owasp-juice.shop/part2/injection.html#exfiltrate-the-entire-db-schema-definition-via-sql-injection',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Deprecated Interface',
                               'https://pwning.owasp-juice.shop/part2/security-misconfiguration.html#use-a-deprecated-b2b-interface-that-was-not-properly-shut-down',
                               2, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Easter Egg',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#find-the-hidden-easter-egg',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Email Leak',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#perform-an-unwanted-information-disclosure-by-accessing-data-cross-domain',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Ephemeral Accountant',
                               'https://pwning.owasp-juice.shop/part2/injection.html#log-in-with-the-non-existing-accountant-without-ever-registering-that-user',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Error Handling',
        ...
        db.session.add(LabItem('Expired Coupon',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#successfully-redeem-an-expired-campaign-coupon-code',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Extra Language',
                               'https://pwning.owasp-juice.shop/part2/broken-anti-automation.html#retrieve-the-language-file-that-never-made-it-into-production',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Five-Star Feedback',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#get-rid-of-all-5-star-customer-feedback',
                               2, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Forged Coupon',
                               'https://pwning.owasp-juice.shop/part2/cryptographic-issues.html#forge-a-coupon-code-that-gives-you-a-discount-of-at-least-80',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Forged Feedback',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#post-some-feedback-in-another-users-name',
                               3, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Forged Review',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#post-a-product-review-as-another-user-or-edit-any-users-existing-review',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Forged Signed JWT',
                               'https://pwning.owasp-juice.shop/part2/vulnerable-components.html#forge-an-almost-properly-rsa-signed-jwt-token',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Forgotten Developer Backup',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#access-a-developers-forgotten-backup-file',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Forgotten Sales Backup',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#access-a-salesmans-forgotten-backup-file',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Frontend Typosquatting',
                               'https://pwning.owasp-juice.shop/part2/vulnerable-components.html#inform-the-shop-about-a-typosquatting-imposter-that-dug-itself-deep-into-the-frontend',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('GDPR Data Erasure',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#log-in-with-chris-erased-user-account',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('GDPR Data Theft',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#steal-someone-elses-personal-data-without-using-injection',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('HTTP-Header XSS',
                               'https://pwning.owasp-juice.shop/part2/xss.html#perform-a-persisted-xss-attack-through-an-http-header',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Imaginary Challenge',
                               'https://pwning.owasp-juice.shop/part2/cryptographic-issues.html#solve-challenge-2', 6,
                               'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Leaked Access Logs',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#dumpster-dive-the-internet-for-a-leaked-password-and-log-in-to-the-original-user-account-it-belongs-to',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Leaked Unsafe Product',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#identify-an-unsafe-product-that-was-removed-from-the-shop-and-inform-the-shop-which-ingredients-are-dangerous',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Legacy Typosquatting',
                               'https://pwning.owasp-juice.shop/part2/vulnerable-components.html#inform-the-shop-about-a-typosquatting-trick-it-has-been-a-victim-of',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Login Admin',
                               'https://pwning.owasp-juice.shop/part2/injection.html#log-in-with-the-administrators-user-account',
                               2, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Login Amy',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#log-in-with-amys-original-user-credentials',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Login Bender',
                               'https://pwning.owasp-juice.shop/part2/injection.html#log-in-with-benders-user-account',
                               3, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Login Bjoern',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#log-in-with-bjoerns-gmail-account',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Login CISO',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#exploit-oauth-20-to-log-in-with-the-cisos-user-account',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(
            LabItem('Login Jim', 'https://pwning.owasp-juice.shop/part2/injection.html#log-in-with-jims-user-account',
                    3, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Login MC SafeSearch',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#log-in-with-mc-safesearchs-original-user-credentials',
                               2, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Login Support Team',
                               'https://pwning.owasp-juice.shop/part2/security-misconfiguration.html#log-in-with-the-support-teams-original-user-credentials',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Manipulate Basket',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#put-an-additional-product-into-another-users-shopping-basket',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Misplaced Signature File',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#access-a-misplaced-siem-signature-file',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Multiple Likes',
                               'https://pwning.owasp-juice.shop/part2/broken-anti-automation.html#like-any-review-at-least-three-times-as-the-same-user',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Nested Easter Egg',
                               'https://pwning.owasp-juice.shop/part2/cryptographic-issues.html#apply-some-advanced-cryptanalysis-to-find-the-real-easter-egg',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('NoSQL DoS',
                               'https://pwning.owasp-juice.shop/part2/injection.html#let-the-server-sleep-for-some-time',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('NoSQL Exfiltration',
                               'https://pwning.owasp-juice.shop/part2/injection.html#all-your-orders-are-belong-to-us',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('NoSQL Manipulation',
                               'https://pwning.owasp-juice.shop/part2/injection.html#update-multiple-product-reviews-at-the-same-time',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Outdated Whitelist',
                               'https://pwning.owasp-juice.shop/part2/unvalidated-redirects.html#let-us-redirect-you-to-one-of-our-crypto-currency-addresses',
                               1, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Password Strength',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#log-in-with-the-administrators-user-credentials-without-previously-changing-them-or-applying-sql-injection',
                               2, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Payback Time',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#place-an-order-that-makes-you-rich',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Premium Paywall',
                               'https://pwning.owasp-juice.shop/part2/cryptographic-issues.html#unlock-premium-challenge-to-access-exclusive-content',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Privacy Policy',
                               'https://pwning.owasp-juice.shop/part2/miscellaneous.html#read-our-privacy-policy', 1,
                               'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Privacy Policy Inspection',
                               'https://pwning.owasp-juice.shop/part2/security-through-obscurity.html#prove-that-you-actually-read-our-privacy-policy',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Product Tampering',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#change-the-href-of-the-link-within-the-o-saft-product-description',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(
            LabItem('Reflected XSS', 'https://pwning.owasp-juice.shop/part2/xss.html#perform-a-reflected-xss-attack', 2,
                    'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Repetitive Registration',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#follow-the-dry-principle-while-registering-a-user',
                               1, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Reset Benders Password',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#reset-benders-password-via-the-forgot-password-mechanism',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Reset Bjoerns Password',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#reset-the-password-of-bjoerns-internal-account-via-the-forgot-password-mechanism',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Reset Jims Password',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#reset-jims-password-via-the-forgot-password-mechanism',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Reset Mortys Password',
                               'https://pwning.owasp-juice.shop/part2/broken-anti-automation.html#reset-mortys-password-via-the-forgot-password-mechanism',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Retrieve Blueprint',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#deprive-the-shop-of-earnings-by-downloading-the-blueprint-for-one-of-its-products',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('SSRF',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#request-a-hidden-resource-on-server-through-server',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('SSTi',
                               'https://pwning.owasp-juice.shop/part2/injection.html#infect-the-server-with-juicy-malware-by-abusing-arbitrary-command-execution',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Score Board',
                               'https://pwning.owasp-juice.shop/part2/score-board.html#find-the-carefully-hidden-score-board-page',
                               1, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Security Policy',
                               'https://pwning.owasp-juice.shop/part2/miscellaneous.html#behave-like-any-white-hat-should-before-getting-into-the-action',
                               2, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Server-side XSS Protection',
                               'https://pwning.owasp-juice.shop/part2/xss.html#perform-a-persisted-xss-attack-bypassing-a-server-side-security-mechanism',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Steganography',
                               'https://pwning.owasp-juice.shop/part2/security-through-obscurity.html#rat-out-a-notorious-character-hiding-in-plain-sight-in-the-shop',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Successful RCE DoS',
                               'https://pwning.owasp-juice.shop/part2/insecure-deserialization.html#perform-a-remote-code-execution-that-occupies-the-server-for-a-while-without-using-infinite-loops',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Supply Chain Attack',
                               'https://pwning.owasp-juice.shop/part2/vulnerable-components.html#inform-the-development-team-about-a-danger-to-some-of-their-credentials',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Two Factor Authentication',
                               'https://pwning.owasp-juice.shop/part2/broken-authentication.html#solve-the-2fa-challenge-for-user-wurstbrot',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Unsigned JWT',
                               'https://pwning.owasp-juice.shop/part2/vulnerable-components.html#forge-an-essentially-unsigned-jwt-token',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Upload Size',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#upload-a-file-larger-than-100-kb',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Upload Type',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#upload-a-file-that-has-no-pdf-or-zip-extension',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('User Credentials',
                               'https://pwning.owasp-juice.shop/part2/injection.html#retrieve-a-list-of-all-user-credentials-via-sql-injection',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Video XSS',
                               'https://pwning.owasp-juice.shop/part2/xss.html#embed-an-xss-payload-into-our-promo-video',
                               6, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('View Basket',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#view-another-users-shopping-basket',
                               2, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(LabItem('Vulnerable Library',
                               'https://pwning.owasp-juice.shop/part2/vulnerable-components.html#inform-the-shop-about-a-vulnerable-library-it-is-using',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Weird Crypto',
                               'https://pwning.owasp-juice.shop/part2/cryptographic-issues.html#inform-the-shop-about-an-algorithm-or-library-it-should-definitely-not-use-the-way-it-does',
                               2, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Whitelist Bypass',
                               'https://pwning.owasp-juice.shop/part2/unvalidated-redirects.html#enforce-a-redirect-to-a-page-you-are-not-supposed-to-redirect-to',
                               4, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('XXE Data Access',
                               'https://pwning.owasp-juice.shop/part2/xxe.html#retrieve-the-content-of-cwindowssystemini-or-etcpasswd-from-the-server',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('XXE DoS',
                               'https://pwning.owasp-juice.shop/part2/xxe.html#give-the-server-something-to-chew-on-for-quite-a-while',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Zero Stars',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#give-a-devastating-zero-star-feedback-to-the-store',
                               1, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Missing Encoding',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#retrieve-the-photo-of-bjoerns-cat-in-melee-combat-mode',
                               1, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Cross-Site Imaging',
                               'https://pwning.owasp-juice.shop/part2/injection.html#stick-cute-cross-domain-kittens-all-over-our-delivery-boxes',
                               5, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Exposed Metrics',
                               'https://pwning.owasp-juice.shop/part2/sensitive-data-exposure.html#find-the-endpoint-that-serves-usage-data-to-be-scraped-by-a-popular-monitoring-system',
                               1, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Deluxe Fraud',
                               'https://pwning.owasp-juice.shop/part2/improper-input-validation.html#obtain-a-deluxe-membership-without-paying-for-it',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('CSRF',
                               'https://pwning.owasp-juice.shop/part2/broken-access-control.html#change-the-name-of-a-user-by-performing-cross-site-request-forgery-from-another-origin',
                               3, 'juice-shop', 'juice-shop', None))
        db.session.commit()

        db.session.add(LabItem('Bonus Payload',
                               'https://pwning.owasp-juice.shop/part2/xss.html#use-the-bonus-payload-in-the-dom-xss-challenge',
                               1, 'juice-shop', 'juice-shop', True))
        db.session.commit()

        db.session.add(
            LabItem('Juice Shop CTF', 'https://owasp.org/www-project-juice-shop/', 3, 'juice-shop-ctf', 'juice-shop',
                    None))
        db.session.commit()
