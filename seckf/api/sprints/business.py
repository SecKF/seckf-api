...
def get_sprint_results(sprint_id):
    ...
    result = ChecklistResult.query.filter(ChecklistResult.sprint_id == sprint_id).order_by(
        asc(ChecklistResult.checklist_id)).paginate(1, 2500, False)
    return result
...
def stats_sprint(project_id):
    ...
    for result in sprint_info:
        ...
        sprint.append(
            {'sprint_id': sprint_id, 'sprint_desc': sprint_desc, 'title': sprint_name, 'sprint_items_total': total})
    return sprint
...
def export_results(sprint_results):
    results = ChecklistResult.query.filter(ChecklistResult.sprint_id == sprint_results).order_by(
        ChecklistResult.checklist_type_id).all()
