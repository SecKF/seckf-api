...
def get_checklist_item_types(category_id):
    ...
    result = ChecklistType.query.filter(ChecklistType.checklist_category_id == category_id).order_by(
        desc(ChecklistType.visibility)).paginate(1, 2500, False)
    return result
...
def get_checklist_item_types_with_filter(maturity):
    log("User requested list checklist types", "LOW", "PASS")
    result = ChecklistType.query.filter(ChecklistType.maturity == maturity).filter(
        ChecklistType.visibility == 1).paginate(1, 2500, False)
    return result
...
def get_checklist_items(checklist_type):
    ...
    result = ChecklistKB.query.filter(ChecklistKB.checklist_type == checklist_type).order_by(
        ChecklistKB.checklist_id.asc()).paginate(1, 2500, False)
    return result
...
def create_checklist_item(checklist_type, data):
    ...
    if validate_duplicate_checklist_item(data.get('checklist_id'), checklist_type) == True:
        try:
            checklist_item = ChecklistKB(data.get('checklist_id'), data.get('content'), checklist_type, include_always,
                                         data.get('add_resources'), data.get('maturity'))
...
def validate_duplicate_checklist_item(checklist_id, checklist_type):
    checklists = ChecklistKB.query.filter(ChecklistKB.checklist_id == checklist_id).filter(
        ChecklistKB.checklist_type == checklist_type).all()
    check = True
