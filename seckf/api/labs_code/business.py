...
from seckf.database.lab_items_code_finished_by_user import LabItemCodeFinishedByUser
...
def get_labs_code_sol(solutions_id):
    ...
    result = LabItemCodeOptions.query.filter(
        (LabItemCodeOptions.id == solutions_id) | (LabItemCodeOptions.id == rand_sol[1]) | (
                LabItemCodeOptions.id == rand_sol[2]) | (LabItemCodeOptions.id == rand_sol[3])).paginate(1, 2500,
                                                                                                         False)
    return result
